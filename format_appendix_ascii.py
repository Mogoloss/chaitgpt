...
path = Path(r"C:\Users\mgq56\Desktop\appendix_work.docx")
...
# Based on current stable file in ASCII temp:
# appendix title at 160, appendix content/spurious line at 161-179, acknowledgements start at 180.
title_para = paras[159]
template_para = paras[160]
ack_para = paras[179]

for para in paras[160:179]:
    body.remove(para)
...
insert_index = list(body).index(ack_para)
for offset, line in enumerate([make_para(template_para, line) for line in lines]):
    body.insert(insert_index + offset, line)
